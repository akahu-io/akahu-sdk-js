**akahu v2.0.0** • **Docs**

***

# akahu v2.0.0

## Classes

### API client

- [AkahuClient](classes/AkahuClient.md)

### Error

- [AkahuErrorResponse](classes/AkahuErrorResponse.md)
- [AkahuWebhookValidationError](classes/AkahuWebhookValidationError.md)

### Resource

- [AccountsResource](classes/AccountsResource.md)
- [AuthResource](classes/AuthResource.md)
- [CategoriesResource](classes/CategoriesResource.md)
- [ConnectionsResource](classes/ConnectionsResource.md)
- [IdentitiesResource](classes/IdentitiesResource.md)
- [PartiesResource](classes/PartiesResource.md)
- [PaymentsResource](classes/PaymentsResource.md)
- [TransactionsResource](classes/TransactionsResource.md)
- [TransfersResource](classes/TransfersResource.md)
- [UsersResource](classes/UsersResource.md)
- [WebhooksResource](classes/WebhooksResource.md)

## Interfaces

### API client

- [WebhookSigningKeyCache](interfaces/WebhookSigningKeyCache.md)

## Type Aliases

### API client config

- [AkahuClientConfig](type-aliases/AkahuClientConfig.md)
- [WebhookCacheConfig](type-aliases/WebhookCacheConfig.md)

### Generic

- [Paginated](type-aliases/Paginated.md)
- [Cursor](type-aliases/Cursor.md)

### Other

- [Account](type-aliases/Account.md)
- [AuthorizationToken](type-aliases/AuthorizationToken.md)
- [Category](type-aliases/Category.md)
- [Connection](type-aliases/Connection.md)
- [PostRequestOptions](type-aliases/PostRequestOptions.md)
- [IdentityResult](type-aliases/IdentityResult.md)
- [IdentityVerifyNameQuery](type-aliases/IdentityVerifyNameQuery.md)
- [AccountHolderNameVerificationSource](type-aliases/AccountHolderNameVerificationSource.md)
- [PartyNameVerificationSource](type-aliases/PartyNameVerificationSource.md)
- [IdentityVerifyNameResult](type-aliases/IdentityVerifyNameResult.md)
- [PaymentStatus](type-aliases/PaymentStatus.md)
- [PaymentStatusCode](type-aliases/PaymentStatusCode.md)
- [Payment](type-aliases/Payment.md)
- [PaymentCreateParams](type-aliases/PaymentCreateParams.md)
- [IrdPaymentCreateParams](type-aliases/IrdPaymentCreateParams.md)
- [PaymentQueryParams](type-aliases/PaymentQueryParams.md)
- [TransactionType](type-aliases/TransactionType.md)
- [RawTransaction](type-aliases/RawTransaction.md)
- [PendingTransaction](type-aliases/PendingTransaction.md)
- [CurrencyConversion](type-aliases/CurrencyConversion.md)
- [EnrichedTransaction](type-aliases/EnrichedTransaction.md)
- [Transaction](type-aliases/Transaction.md)
- [TransactionQueryParams](type-aliases/TransactionQueryParams.md)
- [TransferStatus](type-aliases/TransferStatus.md)
- [Transfer](type-aliases/Transfer.md)
- [TransferCreateParams](type-aliases/TransferCreateParams.md)
- [TransferQueryParams](type-aliases/TransferQueryParams.md)
- [User](type-aliases/User.md)
- [WebhookType](type-aliases/WebhookType.md)
- [WebhookStatus](type-aliases/WebhookStatus.md)
- [Webhook](type-aliases/Webhook.md)
- [WebhookCreateParams](type-aliases/WebhookCreateParams.md)
- [BasePayload](type-aliases/BasePayload.md)
- [CancelledPayload](type-aliases/CancelledPayload.md)
- [TokenPayload](type-aliases/TokenPayload.md)
- [AccountPayload](type-aliases/AccountPayload.md)
- [TransactionPayload](type-aliases/TransactionPayload.md)
- [TransferPayload](type-aliases/TransferPayload.md)
- [PaymentPayload](type-aliases/PaymentPayload.md)
- [WebhookPayload](type-aliases/WebhookPayload.md)
- [WebhookEvent](type-aliases/WebhookEvent.md)
- [WebhookEventQueryParams](type-aliases/WebhookEventQueryParams.md)
- [Protocol](type-aliases/Protocol.md)

### Parties

- [PartyName](type-aliases/PartyName.md)
- [PartyDob](type-aliases/PartyDob.md)
- [PartyPhoneNumber](type-aliases/PartyPhoneNumber.md)
- [PartyEmail](type-aliases/PartyEmail.md)
- [PartyAddress](type-aliases/PartyAddress.md)
- [PartyTaxNumber](type-aliases/PartyTaxNumber.md)
- [Party](type-aliases/Party.md)
